...
pcl_pos = [i - width / 2 for i in n_pos]
amt_pos = [i + width / 2 for i in n_pos]
plt.bar(pcl_pos, hbavss_polycommitloglin_overhead, width, label="pcl + hbavss")
plt.bar(amt_pos, hbavss_amt_overhead, width, label="amt + hbavss")
plt.xlabel("Total players (n=3t+1)")
plt.ylabel("Amortized end-to-end time (seconds)")
plt.title("Simulated AMT and PCL for HbAVSS end-to-end time")
plt.xticks(n_pos, n_vals)
plt.legend(loc="best")
