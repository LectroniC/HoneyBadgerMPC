...
class HbAvssBatchLoglinDummy(HbAvssBatchLoglin):
    def __init__(
...
class HbAvssBigBatchRecoveryDummy(HbAvssBigBatchRecovery):
    def __init__(
            self, public_keys, private_key, crs, n, t, my_id, send, recv, pc=None, field=pypairing.ZR
    ):  # (# noqa: E501)
        self.public_keys, self.private_key = public_keys, private_key
...
class HbAvssBigBatchRecoveryReceivedDummyCaseFaulty(HbAvssBigBatchRecoveryDummy):
    async def _process_avss_msg(self, avss_id, dealer_id, rbc_msg, avid):
            tag = f"{dealer_id}-{avss_id}-B-AVSS"
            send, recv = self.get_send(tag), self.subscribe_recv(tag)
...
async def hbavss_bigbatchrecovery_pcl_one_faulty_share(benchmark_router, params):
    ...
    with ExitStack() as stack:
        ...
        for i in range(n):
            hbavss = None
            if i != fault_i:
                hbavss = HbAvssBigBatchRecoveryDummy(pks, sks[i], crs, n, t, i, sends[i], recvs[i], pc=pcl)
            else:
                hbavss = HbAvssBigBatchRecoveryReceivedDummyCaseFaulty(pks, sks[i], crs, n, t, i, sends[i], recvs[i], pc=pcl)
            hbavss_list[i] = hbavss
            stack.enter_context(hbavss)
            if i == dealer_id:
                avss_tasks[i] = asyncio.create_task(hbavss.avss(0, values=values))
